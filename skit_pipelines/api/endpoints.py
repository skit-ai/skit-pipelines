...
def create_pipeline_run(method_fn: str = "create_run_from_pipeline_func", *args, **kwargs):
    global KF_CLIENT
    try:
...
def tag_calls_req(
    ...
    completed_resp = run_resp.wait_for_run_completion()
    parsed_resp = ParseRunResponse(run=completed_resp, component_display_name=const.TAG_CALLS_NAME)
    if parsed_resp.success:
        return custom_response({
            "message": "Run completed successfully.",
            "run_url": parsed_resp.url,
...
#TODO: endpoint which will be used to keep continuous check using run id to see if pipeline finished (get_run)
