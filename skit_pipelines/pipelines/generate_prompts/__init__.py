...
def generate_prompts(
    *,
    ...
    zip_file_or_folder_data = zip_file_or_folder_op(
        path_on_disk=prompt_generation.outputs["output"]).after(prompt_s3_upload)
    
        
    with kfp.dsl.Condition(notify != "", "notify").after(prompt_s3_upload, zip_file_or_folder_data) as prompt_s3_check:
    ...
    with kfp.dsl.Condition(notify != "", "notify").after(zip_file_or_folder_data) as zip_file_check:
        notification_text_2 = "Here is the ZIP file generated by the Generate Prompt Pipeline."
        zip_notif = slack_notification_op(message=notification_text_2,
                                          channel=channel,
                                          cc=notify,
                                          thread_id=slack_thread,
                                          file_title='ZIP file from Generate Prompt Pipeline.',
                                          file_path = zip_file_or_folder_data.outputs["output"], 
                                          file_type = 'zip', 
                                          file_name = "Generated_data")
        zip_notif.execution_options.caching_strategy.max_cache_staleness = (
            "P0D"  # disables caching
        )
