...
def prod_slu_inference_func(
    slu_repo_tar_path: InputPath(str), s3_tagged_data_path: str, project_id: int
) -> None:
    ...
    for item in items_inside_tar:
        ...
        if os.path.isdir(path_to_item) and any(
            [branch_name in item for branch_name in branch_names]
        ):
    ...
    # dvc pull for models
    proc = subprocess.run("dvc pull", shell=True, check=True)
    print(f"[STDOUT] :: \n {proc.stdout.decode()}")
    print(f"[STDERR] :: \n {proc.stderr.decode()}")
    ...
    df["response"] = df[tagged_region_entity_text_column_name].apply(
        lambda x: api.PREDICT_API(
            alternatives=x,
            # context=context,
