...
def adjust_interval_values(interval_value: Dict):
    ...
    if (isinstance(interval_value.get("from"), dict) and interval_value.get("from").get("value")) \
        or (isinstance(interval_value.get("to"), dict) and interval_value.get("to").get("value")):
