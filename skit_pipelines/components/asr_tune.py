...
def asr_tune(
    ...
    def exec_shell(cmd: str, tolerant: bool=False):
        ...
        code = subprocess.call("source ~/.bashrc && " + cmd, shell=True, executable='/bin/bash', stdout=sys.stdout, stderr=sys.stderr)
        if code:
            if not tolerant:
    ...
    # can't use this condition here as base_model_path is just some local path.
    # assert (
    #     "full-new-structure" in base_model_path
    # ), "base model needs to be `full-new-structure`"
    exec_shell(f"wget https://repo.anaconda.com/miniconda/Miniconda3-py37_4.12.0-Linux-x86_64.sh && bash Miniconda3-py37_4.12.0-Linux-x86_64.sh -b")
    ...
    exec_shell("echo 'export PYTHONIOENCODING=utf8 && export LC_ALL=C.UTF-8 && export LANG=C.UTF-8' >> ~/.bashrc")
    ...
    exec_shell(f"cd ~/corprep && python ~/kaldi/scripts/corpus/fix_corpus.py ~/corprep/data/processed/{LANGS[lang]}.yaml {recipe_path}/data/local/corpus.txt")
    exec_shell(f"cd ~/corprep && poetry run python ~/kaldi/scripts/corpus/fix_corpus.py ~/corprep/data/processed/{LANGS[lang]}.yaml {recipe_path}/data/local/corpus.val.txt")
    exec_shell(f"cd {recipe_path}/data/local/ && python3 ~/kaldi/scripts/corpus/uniq_words.py corpus.txt")
    ...
    exec_shell(f"cd {recipe_path}/data/local && less dict/lexicon.txt | grep -v '<SIL>' | grep -v '<UNK>' > dict/lexicon_filt.txt")
    exec_shell(f"cd {recipe_path}/data/local && python3 ~/kaldi/scripts/lexicon/lexicon_txt_to_csv.py dict/lexicon_filt.txt")
    exec_shell(f"cd {recipe_path}/data/local && cp dict/lexicon_filt.csv dict/lexicon.csv")
    ...
    if lang == "en":
        ...
        exec_shell(f"cd ~/g2p && poetry run python3 ~/kaldi/scripts/lexicon/words_to_lexicon.py {recipe_path}/data/local/{augment_wordlist} {recipe_path}/data/local/lexicons/lexicon.csv --sequitur-model {PHONEMIZERS[lang]}")
    elif lang == "hi":
        exec_shell(f"mkdir -p {recipe_path}/data/local/lexicons")
        exec_shell(f"cd ~/g2p && pip3 install -r ~/kaldi/requirements.txt")
        exec_shell(f"cd ~/g2p && poetry run python3 ~/kaldi/scripts/lexicon/words_to_lexicon.py {recipe_path}/data/local/{augment_wordlist} {recipe_path}/data/local/lexicons/ext.lexicon.csv --unified-parser {PHONEMIZERS[lang]}")
    else:
        raise UserWarning(
            "language specified does not have phonemization configured"
        )
    exec_shell(f"cd {recipe_path}/data/local && cp dict/lexicon.csv lexicons/orig.lexicon.csv")
    exec_shell(f"cd {recipe_path}/data/local && python3 ~/kaldi/scripts/lexicon/merge_lexicons.py lexicons lexicon_ext.csv")
    ...
    exec_shell(f"cd {recipe_path}/data/local && python ~/kaldi/scripts/lexicon/remove_words.py --inplace tmp_lexicon.csv {remove_wordlist_path}")
    exec_shell(f"cd {recipe_path}/data/local && cp -nf tmp_lexicon.csv lexicon_ext.csv")
    
    dict_dir = f"dict_ext"
    exec_shell(f"cd {recipe_path}/data/local")
    exec_shell(f"cd {recipe_path}/data/local && python3 ~/kaldi/scripts/lexicon/lexicon_csv_to_txt.py lexicon_ext.csv")
    exec_shell(f"cd ~/kaldi")
    exec_shell(f"mkdir -p {recipe_path}/data/local/{dict_dir}")
    exec_shell(f'cd ~/kaldi && python3 -c "import scripts.artefacts.prepare_dict as prep; prep.gen_phones(\\"recipes/{RECIPE}/data/local/lexicon_ext.txt\\",\\"recipes/{RECIPE}\\",dict=\\"{dict_dir}\\")"')

    
    exec_shell(f"cd {recipe_path}")
    exec_shell(f"cd {recipe_path} && cp data/local/corpus.txt data/local/corpus_ext.txt")
    exec_shell(f"mkdir ~/kaldi/tools/extras/ && cp ~/kaldi/extras/install_liblbfgs.sh ~/kaldi/tools/extras/")
    exec_shell(f"cd ~/kaldi/tools && ./install_srilm.sh somename someorg someuser@somedomain.com")
    exec_shell("echo 'source ~/kaldi/tools/env.sh' >> ~/.bashrc")
    exec_shell("apt update && apt install -y libatlas-base-dev && apt install -y bsdutils && apt install -y bsdmainutils")
    exec_shell(f"ls {recipe_path}/data/local/lm_general.arpa", tolerant=True)
    exec_shell(f"ls -lat {recipe_path}/data/local/tmp_general_100k/lm_general.arpa", tolerant=True)
    exec_shell(f"ls -lat {recipe_path}/data/local/tmp_ext/lm.arpa", tolerant=True)
    exec_shell(f"ls -lat {recipe_path}/data/local/tmp_domain_ext/lm.arpa", tolerant=True)
    exec_shell(f"ls -lat {recipe_path}/data/local/tmp_ext/", tolerant=True)
    exec_shell(f"cd {recipe_path} && ./lm/tune_lm_domain.sh --mode ext --val-text corpus.val.txt --general-lm data/local/lm_general.arpa")

    # recipe_path = f"~/kaldi/recipes/{RECIPE}"

    # graph creation: Creates final HCLG.fst graph (with new L,G.fst)
    exec_shell(f"cd {recipe_path}")
    exec_shell(f"cd {recipe_path} && ./scripts/run_test.sh --create-hclg true --test-sets \"\" --lang lang_ext --graph-dir graph_ext")
    ...
    exec_shell(f"cd ~/kaldi && ./scripts/model_to_bucket.sh recipes/{RECIPE} ext tdnn{NNET_SUFFIX[lang]}_sp_online tree_a_sp AWS {target_uri}", tolerant=True)
